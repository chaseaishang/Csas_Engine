...
vec3 PrefilterEnvironmentMap(vec3 R, uint n_samples) {
    vec3 N = R;
    vec3 V = R;
//镜面的 BRDF 大致会在反射反向R=reflect(-WO,N) 附近有值
    ...
    for(uint i = 0u; i < n_samples; ++i)
    {
        ...
        if(NoL > 0.0)
        {
            ...
            float pdf = DistributionGGX(N, H, roughness) * NoH / (4.0 * HoV);
            ...
            prefilteredColor += textureLod(environment_map, L, mip_level).rgb * NoL;
            totalWeight      += NoL;
        }
    }
...
void main() {
    ...
    vec3 color = PrefilterEnvironmentMap(N, 1024);  // ~ 1024 samples for 2K resolution
